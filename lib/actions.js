...
module.exports = {
  ...
  addPlugin: async (name) => {
    ...
    if (fs.existsSync(path.join(process.cwd(), './core/Plugins', name))) {
      fs.rmdirSync(path.join(process.cwd(), './core/Plugins', name), { recursive: true });
    }

    try {
      cloneOptions.ignoreCertErrors = 1;
      cloneOptions.checkoutBranch = 'master';
      await clone(`https://github.com/alexdant91/nodemvc-${nameParsed}.git`, path.join(process.cwd(), './core/Plugins', name), cloneOptions);
  ...
  plugins: [
    "${name}",
      ...
      fs.writeFile(path.join(process.cwd(), './nodemvc.config.js'), nodeMvcConfigTemplate, () => {
        ...
        const packageJSON = require(path.join(process.cwd(), './core/Plugins', name, './package.json')).dependencies;
        const command = `yarn add ${Object.keys(packageJSON).join(' && yarn add ')}`;
  ...
  plugins: [
    "${name}",
      ...
      fs.writeFile(path.join(process.cwd(), './nodemvc.config.js'), nodeMvcConfigTemplate, () => {
        ...
        const packageJSON = require(path.join(process.cwd(), './core/Plugins', name, './package.json')).dependencies;
        const command = `yarn remove ${Object.keys(packageJSON).join(' && yarn remove ')}`;
